...
- **Styling keyed to a Gradio internal.** Its component classes carry a `svelte-<hash>`
  that changes on every release, so our tab rules are keyed to ARIA roles instead, and a
  test fails if a hash ever appears in the stylesheet.
